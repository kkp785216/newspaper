    ...
    <div>
      ...
      {articles.map((e, i) => (
        <div className="flex space-x-4 md:space-x-3 lg:space-x-4 group mb-7 md:mb-6" title={e.title} key={i}>
          <Links className="w-1/3 md:w-[30%]" to={`/${e.url}`}><LazyLoad><img className="w-full object-cover" src={e.img_url ? e.img_url : `/img/articles/218x150/${e.img_comp}.jpg`} alt={e.title} /></LazyLoad></Links>
