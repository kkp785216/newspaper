  ...
  return (
    <div className="group w-full md:w-1/2 px-5 md:px-[10px] lg:px-5 py-4" title={article.title}>
      <Link href={`/${article.url}`} className="relative w-full block overflow-hidden pb-[50%]">
        <Image className='absolute top-0 left-0 right-0 bottom-0 w-full h-full object-cover transition-all duration-[.5s]' layout='fill' sizes='485px' src={article.img_url ? article.img_url : `/img/articles/485x360/${article.img_comp}.jpg`} alt={article.title}></Image>
