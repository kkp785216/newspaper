            ...
            <div>
                {most_popular.articles.filter(e => e.page === most_popular.current_page).map((e, i) => (
                    <div className="flex space-x-4 md:space-x-3 lg:space-x-4 group mb-7 md:mb-4" title={e.title} key={i}>
                        <Link className="w-1/3" to={`/${e.url}`}><LazyLoad><img className="w-full object-cover" src={e.img_url ? e.img_url : `/img/articles/218x150/${e.img_comp}.jpg`} alt={e.title} /></LazyLoad></Link>
                        <div className="w-2/3">
