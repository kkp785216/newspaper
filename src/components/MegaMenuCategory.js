...
const MegaMenuCategory = ({ config_menu }) => {

  const { mega_menu_category } = useSelector(state => state);
  const [page, setPage] = useState(1)
  const dispatch = useDispatch();

  useEffect(() => {
    !mega_menu_category[config_menu.url] &&
      dispatch(action({
    ...
    mega_menu_category[config_menu.url] &&
      !mega_menu_category[config_menu.url].pages_loaded.includes(page) &&
      dispatch(action({
        type: 'MEGA_MENU_CATEGORY',
        url: config_menu.url,
        page: page
  ...
  const handlePageClick = (button) => {
    if (mega_menu_category[config_menu.url] && page >= 2 && page <= Math.ceil(mega_menu_category[config_menu.url].total_articles / 5) && button === "prev") {
      setPage(page - 1);
    }
    if (mega_menu_category[config_menu.url] && page >= 1 && page < Math.ceil(mega_menu_category[config_menu.url].total_articles / 5) && button === "next") {
      setPage(page + 1);
    }
  }
      ...
      <div className='w-full p-6 border-r'>
        <div className=' flex -m-3'>
          {mega_menu_category[config_menu.url] && mega_menu_category[config_menu.url].pages_loaded.includes(mega_menu_category[config_menu.url].current_page) && mega_menu_category[config_menu.url].articles.filter(e => e.page === mega_menu_category[config_menu.url].current_page).map((e, i) => (
            <div className="group w-1/4 p-3" key={i}>
        ...
        {mega_menu_category[config_menu.url] && <div className="flex space-x-2 mt-5">
          ...
          <button onClick={e => handlePageClick('next')} className="border p-1.5 hover:bg-blue-500 hover:border-blue-500 transition-all duration-150 text-[#b7b7b7] hover:text-white disabled:opacity-60 disabled:hover:bg-white disabled:hover:text-[#b7b7b7] disabled:hover:border-[#e5e7eb]" disabled={page < 1 || page >= Math.ceil(mega_menu_category[config_menu.url].total_articles / 5)}>
