...
const Article = ({ title, img_url, img_comp, date, url, author }) => {
  ...
  const lazyImg = useRef();

  return (
    <div className="group w-1/2 px-5 py-4" title={title}>
      <Link to={`/${url}`} className="relative w-full block overflow-hidden pb-[50%]">
        <LazyLoad><img ref={lazyImg} className="absolute top-0 left-0 right-0 bottom-0 transition-all duration-[.5s]" src={img_url ? img_url : `./img/articles/485x360/${img_comp}.jpg`} alt={title} /></LazyLoad>
