...
const Article = ({ title, img_url, img_comp, date, url, author }) => {
  ...
  return (
    <div className="group w-1/2 px-5 md:px-[10px] lg:px-5 py-4" title={title}>
      <Link to={`/${url}`} className="relative w-full block overflow-hidden pb-[50%]">
        <LazyLoad><img className="absolute top-0 left-0 right-0 bottom-0 transition-all duration-[.5s]" src={img_url ? img_url : `/img/articles/485x360/${img_comp}.jpg`} alt={title} /></LazyLoad>
      ...
      <h3 className="text-21px md:text-[16.7px] md:font-base leading-7 md:leading-6 lg:leading-7 mt-3 group-hover:text-sky-400"><Link to={`/${url}`}>{title}</Link></h3>
